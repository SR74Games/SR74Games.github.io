...
function drawVertexListOpen(ctx, vertexList) { // Draw the animated vertex list for the specified position.
   var position = currentTime;
   var startPoint = getValue(vertexList[0].v, position);
   ctx.beginPath();
   ctx.moveTo(startPoint[0], startPoint[1]);
   if (vertexList.length == 2) {
     var endPoint = getValue(vertexList[1].v, position);
      ctx.lineTo(endPoint[0], endPoint[1]);
   } else if (vertexList.length == 1) {
      var endPoint = getValue(vertexList[0].cn, position);
      ctx.lineTo(endPoint[0], endPoint[1]);
   } else {
      ...
      vertexPrev = vertexList[0];
      cp1 = getValue(vertex.cn, position);
      cp2 = getValue(vertex.v, position);
      var vend = getValue(vertexPrev.cp, position);
      ctx.bezierCurveTo(cp1[0], cp1[1], cp2[0], cp2[1], vend[0], vend[1]);
   }
}
...
function drawShape(ctx, shape) { // Draws a shape
   ...
   ctx.save(); // Keep the state of the context, since with radial gradient, it may be necessary to change the transform matrix
   if (shape.openfigure != null) drawVertexListOpen(ctx, shape.vertexList, position);
   else drawVertexList(ctx, shape.vertexList, position);
...
var AN_TEXT_OBJECT_SCALE = 0.01;

function getTextValue(strings, position) {
   if (strings.length == 0) return; // Error: animation must always have at least 1 key.
   position -= nestedCompositionOffset;

   var best = 0;
   for (var i = 0; i < strings.length; i++) {
      if (strings[i].time > position) break;
      best = i;
   }
   return strings[best].text;
}

function getPlainTextValue(text) {
   var whole = "";
   for (var i = 0; i < text.length; i++) {
      whole += text[i].subtext;
   }
...
function getMaxLineWidth(ctx, lines) {
   ...
   for (var i = 0; i < lines.length; i++) {
      var textSize = ctx.measureText(lines[i]).width;
      if (textSize > maxWidth) maxWidth = textSize;
   }
   return maxWidth;
}

function drawText(ctx, text) { // Draw a text object
   var position = currentTime;
   ctx.save();
   // The baseline for text "bottom"
   ctx.textBaseline = "bottom";
   ctx.font = text.font;
   ctx.textAlign = 'left';
   var defaultColor = valueToColor(getValue(text.color, position));

   ctx.scale(AN_TEXT_OBJECT_SCALE, AN_TEXT_OBJECT_SCALE); // Apply text object scale

   var sText = getTextValue(text.strings, position);
   var sPlainText = getPlainTextValue(sText);
   var Lines = sPlainText.split("\n");
   var maxWidth = getMaxLineWidth(ctx, Lines);

   var line = 0;
   var x = 0;
   if (text.alignment == "center") {
      x = (maxWidth - ctx.measureText(Lines[line]).width) / 2;
   } else if (text.alignment == "right") {
      x = maxWidth - ctx.measureText(Lines[line]).width;
   }

   var underlineSize = text.lineHeight / 20.0; // TODO: Use the correct underline size value, for now just calculate it based on the text height.
   var strokeWidth = getValue(text.strokeWidth, position)[0];

   var y = 0;
   for (var i = 0; i < sText.length; i++) {

      if ("font" in sText[i]) {
         ctx.font = sText[i].font;
      } else ctx.font = text.font;

      if ("color" in sText[i]) {
         ctx.fillStyle = valueToColor(sText[i].color);
      } else ctx.fillStyle = defaultColor;

      var underline = text.underline;
      if ("underline" in sText[i]) underline = sText[i].underline;

      var subText = decodeURI(sText[i].subtext);
      var lines = subText.split("\n");

      for (var iLine = 0; iLine < lines.length; iLine++) {
         ctx.fillText(lines[iLine], x, y);

         var textWidth = ctx.measureText(lines[iLine]).width;

         if (underline > 0) { // Draw text underline
            ctx.strokeStyle = ctx.fillStyle;
            ctx.lineWidth = underlineSize;
            ctx.beginPath();
            ctx.moveTo(x, y + underline);
            ctx.lineTo(x + textWidth, y + underline);
            ctx.stroke();
         }

         if (strokeWidth > 0) { // Draw text stroke if enabled
            var strokeColor = getValue(text.strokeColor, position);
            ctx.strokeStyle = valueToColor(strokeColor);
            ctx.lineWidth = strokeWidth;
            ctx.strokeText(lines[iLine], x, y);

            if (underline > 0) { // Draw stroke around text underline
               ctx.beginPath();
               ctx.rect(x, (y + underline) - (underlineSize / 2.0), textWidth, underlineSize);
               ctx.stroke();
            }
         }

         if ((iLine + 1) < lines.length) {
            line++;
            if (text.alignment == "center") {
               x = (maxWidth - ctx.measureText(Lines[line]).width) / 2;
            } else if (text.alignment == "right") {
